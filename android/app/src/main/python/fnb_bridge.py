...
def _log(msg):
    try:
        print(f"[fnb_bridge] {msg}", flush=True)
    except Exception:
        pass
    ...
    try:
        ...
        for p in [here, assets]:
            if p not in sys.path:
                sys.path.insert(0, p)
        # venv site-packages（开发态 .venv-linux；含 curl_cffi/pydantic 等）
...
def call(function: str, json_args: str = "{}") -> str:
    ...
    try:
        ...
        if eng is None:
            ...
            result = _dispatch(eng, platform, function, a)
    except Exception as e:
...
def _dispatch(eng, platform, function, a):
    ...
    if function in coro_map:
        coro = coro_map[function]()
